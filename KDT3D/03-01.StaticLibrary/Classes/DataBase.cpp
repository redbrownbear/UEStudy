...
FDataBase* FDataBase::Get(const bool bDestroy)
{
...
{
    auto Iter = AccountMap.find(InID);
...
FAccount* FDataBase::CheckAccount(const FAccount& InAccount, EErrorCode* OptionalOutErrorCode)
{
    ...
    if (FoundAccount == nullptr)
    {
        if (OptionalOutErrorCode)
        {
            *OptionalOutErrorCode = EErrorCode::ECanNotFoundAccount;
        }
        return nullptr;
    }
    ...
    if (*FoundAccount != InAccount)
    {
        if (OptionalOutErrorCode)
        {
            *OptionalOutErrorCode = EErrorCode::EAccountNotValid;
        }
        return nullptr;
    }
...
EErrorCode FDataBase::ChangeLevel(const FAccount& InAccount, std::string_view InPlayerName, int InLevel)
{
    ...
    //제이슨 저장
    Player->Save(PlayerInfo, Doc.GetAllocator());
    ...
    FLoginSession::Get()->ChangeLoginInfo(InAccount.ID, *Player);
...
FDataBase::FDataBase()
{
    //실행전 폴더를 미리 만들어 둔다 밑에 for문을 통해 폴더를 찾을때 없을 수 있기 때문에...
