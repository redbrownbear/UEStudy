...
int main()
{
   //new
	...
	// 메모리 할당이란 : 커널에 요청하는 명령(system call)중 하나
	// (OS가 최적화를 해놔 매번 커널에 요청하는 것은 아닐 수 있다.)
	// 왜 커널에 요청하는가? 메모리는 공유 자산(내 프로그램만 쓰는 것이 아니다)
	// 이러한 공유자원을 사용하는 요청은 동기화 처리가 되어야 할 것이다.
	// (동기화란, 여러 프로그램이 동시에 요청할 때 순서대로 하나씩 처리되어야 함)

	// 위와 같은 상황의 경우 최대한 커널 영역에 있는 시간을 줄여야 (OS입장에서)
	// 프로그램이 더 빠르게 동작한다고 할 수 있겠다.
	...
	//		-> 결국 성능을 측정해야 한다.
	//		 - 미리 적은 횟수의 시스템 콜로 메모리 블록을 크게 할당하고 그것을 유저 영역에서 사용하기 때문에 빠르다.
	//		 - (메모리 단편화 문제 해결) 큰 블록을 할당해 두고 이를 유저가 쪼개서 사용하기 때문에 캐시 미스가 적다
	// 고정 사이즈 메모리 풀
	//		 - 예산(메모리)을 정해서 그 예산 안에서 사용하게 제한을 할 수 있다
	//			- 예를 들면 메모리풀 크기를 10으로 고정해두고 이를 사용하면 최대 동시에 10개 까지 만들 수 있다.
	...
	//				오브젝트풀은 placement new를 사용해서 object를 만들어주는 기능이 추가됨
	{
...

		{
			...
			std::cout << std::format("Release: ");
#endif
			std::cout << std::format("shared_ptr: {}ms\n", DiffCount);
		}
		{
			std::vector<std::shared_ptr<FClass>> Vector;
			Vector.reserve(MaxCount);
			auto Start{ std::chrono::steady_clock::now() };
			{
				for (size_t i = 0; i < MaxCount; ++i)
				{
					std::allocator<FClass> Alloctor;
					std::shared_ptr<FClass> Instance = std::allocate_shared<FClass, std::allocator<FClass>>(Alloctor, i);
					Vector.push_back(Instance);
				}
			}
			...
			std::cout << std::format("Release: ");
#endif
			std::cout << std::format("[FClass] custom alloactor shared_ptr: {}ms\n", DiffCount);
		}
	}
	//Memory Header
	{
