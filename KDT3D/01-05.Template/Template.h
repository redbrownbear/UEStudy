...
public:
	...
	//void SetDefaultWidth(const size_t InWidth) { WIDTH = InWidth; }	수정 불가
...
private:
	...
	friend FStruct operator +(const FStruct& p1, FStruct& p2);
};

FStruct operator +(const FStruct& p1, FStruct& p2)
{
	FStruct temp;
	temp.Value = p1.Value + p2.Value;
	temp.Value2 = p2.Value2 + p2.Value2;
...
//컴파일 타임에 값을 추론 가능한 경우 추론한다.
//그리고, 추론하지 못하는 경우 실행 시간에 계산 한다.
//consteval inline unsigned long long Factorial(unsigned char f)	<- 추론 불가
